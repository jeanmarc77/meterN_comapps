/* 17/06/19 Louviaux Jean-Marc - meterN communication app example for gas and water pulses meters */
...
void loop() {
  ...
  // Gas
  if (Gstat == 1 && now - Glow_time > 30000) { // Clear gas meter state after 30secs
    Gstat = 0;
  }
  ...
  if (Ginputstate != Gprevinputstate) { // Compare to its previous state
    ...
    if (Ginputstate == HIGH && now - Glow_time > 300) { // pulse width 300ms
      Ghigh_time = now;
      Gcount++;
    } else {
      Gerr ++;
  ...
  if (Winputstate != Wprevinputstate) {
    now = millis();
    if (Winputstate == HIGH && now - Wlow_time >= 1000 ) {  // At least 1s between a liter, max 60 l/m
      Wprevhigh_time = Whigh_time;
      Whigh_time = now;
      Wpulse_time = Whigh_time - Wprevhigh_time;
    }
    if (Winputstate == LOW && now - Whigh_time > 300) { // pulse width 300ms
      Wlow_time = now;
      Wcount ++;
    } else {
      Werr ++;
  ...
  if (Serial.available()) {
    ...
    if (inChar == '\n') {
      if (inputString == "ws") { // Water state
        if (now - Wlow_time > 10000) { // Clear water state after 10secs
          Serial.println("water(0*l/min)");
        } else { // liters per min from 6l/m (10sec)
          if (now - Whigh_time > Wpulse_time) { // auto decrease
